import { Kicker } from "@/components/ui/Kicker";

const items = [
  {
    title: "Mayor autonomía",
    body: "Trámites y comunicaciones digitales sin depender de nadie.",
  },
  {
    title: "Menos aislamiento",
    body: "Conexión digital con familia, amigos y comunidad.",
  },
  {
    title: "Acceso a información",
    body: "Servicios locales y contenidos confiables al alcance.",
  },
  {
    title: "Empoderamiento cívico",
    body: "Participación activa en iniciativas vecinales y plataformas ciudadanas.",
  },
];

export function ImpactoGrid() {
  return (
    <section className="mx-auto w-full max-w-5xl px-4 sm:px-6 py-16 sm:py-20">
      <Kicker>■ IMPACTO EN LA COMUNIDAD</Kicker>
      <h2
        className="mt-3 text-xl sm:text-2xl"
        style={{
          fontFamily: "var(--font-display)",
          fontWeight: 800,
          letterSpacing: "-0.035em",
          lineHeight: 1.1,
        }}
      >
        Impacto
      </h2>
      <div className="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-2 sm:gap-6">
        {items.map((it) => (
          <article key={it.title} className="rounded-lg p-5" style={{ backgroundColor: "#111827" }}>
            <h3 className="text-base font-semibold">{it.title}</h3>
            <p className="mt-2 text-sm leading-relaxed" style={{ color: "#A0A0A0" }}>
              {it.body}
            </p>
          </article>
        ))}
      </div>
    </section>
  );
}
